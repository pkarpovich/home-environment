...
`audit.sh` runs after the Sunday `restic check` (ExecStartPost) and compares live host state against the coverage lists: every named docker volume, every compose project dir, and every running postgres/mysql container must be matched by `includes.txt`, `excludes.txt`, a dump hook in `pre-backup.sh`, or an explicit line in `audit-ignore.txt` (with a `# reason`). Anything unmatched goes to telegram via [tg-relay-bot](https://github.com/pkarpovich/tg-relay-bot) (`RELAY_SECRET` in `/etc/restic/env`, URL defaults to `https://relay.pkarpovich.space/send`) - no bot token ever lands on the hosts. The same verdict is pushed to a Gatus external endpoint (`Backups / backup-audit-alpha`, `backup-audit-bravo`, tokens `AUDIT_ALPHA_TOKEN` / `AUDIT_BRAVO_TOKEN` in alpha's `.env` and `GATUS_AUDIT_TOKEN` in each host's `/etc/restic/env`), so drift is a red tile on the dashboard until it is answered rather than one telegram message that scrolls away - which is exactly how the 2026-09-06 report went unfixed until alpha's card died three days later. The 8d `heartbeat.interval` also catches the audit silently not running at all. **The rule: any change that creates persistent state on a host must update these lists in the same PR** - the audit is the safety net for the forgotten case, not a substitute for doing it.
