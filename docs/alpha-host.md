...
- **sysctl** `/etc/sysctl.d/99-thread.conf` disables IPv6. `/etc/sysctl.d/99-tailscale.conf` turns on forwarding for the subnet router and exit node and switches TCP to BBR with a 4 MB initial send buffer and no slow start after idle, which is what makes the DERP relay path usable (see [`tailnet.md`](tailnet.md), "Throughput"); `tcp_bbr` is loaded from `/etc/modules-load.d/tcp_bbr.conf`.
...
1. **home-environment first.** `git.pkarpovich.space` is served by this Traefik and, since 2026-09-13, IS this stack's `gitea` container, and every turtle-hub image is pulled from it, so nothing else can start until Traefik is up. Since 2026-09-14 the same stack also carries `nats` and `seaweedfs` (moved out of turtle-hub): turtle-hub's continuum, twitch and stream-recorder, magnet-feed-sync and tuclaw on bravo all connect to them, by container name over `proxy` or through the host's `:4222` and `:8333`. Restore the `letsencrypt` volume before the first Traefik start or it re-issues every certificate against the Let's Encrypt rate limit.
