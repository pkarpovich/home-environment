...
The default policy is `one_factor`, which a passkey login satisfies on its own; the account password exists for the first sign-in and as the fallback. `info.*` carries the single `two_factor` rule, and that rule is load-bearing: Authelia hides the whole second-factor UI, WebAuthn credential registration included, until at least one policy requires `two_factor`, so without it a passkey could never be registered. The canary therefore asks for one more WebAuthn tap after sign-in; everything else is one tap. Sessions live in `authelia-redis` (AOF-persisted, reachable only over the internal `authelia` network), so restarts and deploys keep you signed in for the `remember_me` window. Persistent state (SQLite + the one-time-code file) is the `authelia` volume, listed in the alpha backup includes; the Redis volume holds sessions only and is audit-ignored, a re-login rebuilds it.
...
Services that speak OIDC (Gitea, Grafana, Linkding, Ryot, ...) get SSO through Authelia's OIDC provider instead of forward-auth, so their own machine traffic - git over HTTPS, container registry pushes, API tokens, CI runners - is untouched and only the browser login changes. The provider's RSA key lives in `authelia/secrets/oidc.pem` and each client's hashed secret in `authelia/secrets/<client>.client_secret`; the directory is git-ignored and read through the `secret` template function. The HMAC secret is `AUTHELIA_IDENTITY_PROVIDERS_OIDC_HMAC_SECRET` in `.env`. Discovery URL: `https://auth.<domain>/.well-known/openid-configuration`.

Adding a client: `docker run --rm ghcr.io/authelia/authelia:4.39 authelia crypto hash generate pbkdf2 --random --random.length 64 --random.charset alphanumeric --no-confirm` prints a password and a digest - the digest goes to `authelia/secrets/<client>.client_secret` on alpha, the password into the application; add a `clients:` entry in `authelia/configuration.yml` with the application's callback as `redirect_uris`, then `docker compose restart authelia`. Consent is `implicit` (single user, own applications).

Grafana (alpha, `compose-grafana.yml`): client id `grafana`, callback `https://grafana.<domain>/login/generic_oauth`, PKCE required, and a `claims_policies.grafana` entry that puts `email`/`name`/`groups`/`preferred_username` into the ID token because Grafana's generic OAuth reads them from there. Grafana side is all environment (`GF_AUTH_GENERIC_OAUTH_*`, plaintext secret as `GRAFANA_OAUTH_CLIENT_SECRET` in `.env`); the login binds to the existing local user by **email** (`GF_AUTH_OAUTH_ALLOW_INSECURE_EMAIL_LOOKUP=true` - Grafana's OAuth user sync matches only by `auth_id` or, with that flag, by email, never by login; "insecure" means trusting the IdP's email claim, which is fine when the IdP is our own Authelia), so the local user's email must equal the Authelia account's; the `admins` group maps to `GrafanaAdmin`, anything else to `Viewer`. The local admin credentials come from `GRAFANA_USER`/`GRAFANA_PASSWORD` in `.env` (shared with the Homepage widget) and stay as the fallback; the Homepage widget, `mcp-grafana` (service-account token) and the Gatus check are untouched because nothing sits in front of the host.

Linkding (alpha, `compose-linkding.yml`, 1.46): client id `linkding`, callback `https://bookmarks.<domain>/oidc/callback/`, PKCE on (linkding's default), `client_secret_post`. Linkding uses the stock mozilla-django-oidc backend, which matches an existing account by its **email** field and otherwise creates a user named after the claim, so the local superuser's email must equal the Authelia account's before the first OIDC sign-in. Plaintext secret as `LINKDING_OIDC_CLIENT_SECRET` in `.env`; the API tokens (browser extension, tuclaw's secret broker) are untouched.

Ryot (alpha, `compose-ryot.yml`, v10): client id `ryot`, callback `https://ryot.<domain>/api/auth`, scopes `openid email`, no PKCE, and a `claims_policies.ryot` entry putting `email` into the ID token - Ryot reads it from there, not from userinfo, and fails with "Email not found in OIDC token claims" otherwise. Ryot matches an OIDC login **only** by the `sub` stored in `user.oidc_issuer_id` and otherwise creates a new user named after the email - it never links by email or name - so the existing local user is pre-linked by hand: `UPDATE "user" SET oidc_issuer_id = '<sub>'` in `ryot-db`, where `<sub>` comes from `docker exec authelia authelia storage user identifiers export --config /config/configuration.yml --file /tmp/ids.yml` (one identifier per user, shared by all clients). Plaintext secret as `RYOT_OIDC_CLIENT_SECRET` in `.env`; `FRONTEND_URL` is required for the callback. Local login stays (no `USERS_DISABLE_LOCAL_AUTH`).

Gitea (on the NAS, proxied at `git.*`): client id `gitea`, callback `https://git.<domain>/user/oauth2/authelia/callback` - the `authelia` path segment is the Authentication Source name, so the two must match. In Gitea: Site Administration -> Identity & Access -> Authentication Sources -> Add: type OAuth2, name `authelia`, provider OpenID Connect, client id/secret from above, auto discovery URL as above. Sign in via the new button once and Gitea offers to link the existing account with its password; no `app.ini` change (no auto-registration wanted, single user).

### Bootstrap (once, on alpha)

1. Three secrets into `.env`, `AUTHELIA_SESSION_SECRET`, `AUTHELIA_STORAGE_ENCRYPTION_KEY` and `AUTHELIA_IDENTITY_PROVIDERS_OIDC_HMAC_SECRET`, each from `docker run --rm ghcr.io/authelia/authelia:4.39 authelia crypto rand --length 64 --charset alphanumeric`; the OIDC signing key from `docker run --rm -v ./authelia/secrets:/keys ghcr.io/authelia/authelia:4.39 authelia crypto pair rsa generate --bits 4096 --directory /keys --file.private-key oidc.pem --file.public-key oidc.pub`.
